...
module Repertoire
  module Faceting
    ...
    module PostgreSQLAdapter #:nodoc:
      ...
      def load_signature_facets
        sql = "CREATE EXTENSION faceting"
        execute(sql)
      end

      # Unloads PostgreSQL native bitset type
      def unload_signature_facets
        sql = "DROP EXTENSION faceting CASCADE"
        execute(sql)
      end

      # Load PostgreSQL basic bitset api into current database
      def load_bit_facets
        [ '/ext/bit/bit.sql', '/ext/utils/utils.sql' ].each do |file|
          sql = File.read(Repertoire::Faceting::MODULE_PATH + file)
          execute(sql)
        end
      end

      # Unloads PostgreSQL native bitset type
      def unload_bit_facets
          [ '/ext/bit/uninstall.sql', '/ext/utils/uninstall.sql' ].each do |file|
            sql = File.read(Repertoire::Faceting::MODULE_PATH + file)
            execute(sql)
          end
      end
